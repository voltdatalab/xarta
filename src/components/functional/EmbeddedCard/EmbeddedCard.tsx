        ...
        <CardFooter className="bg-[#E7E7E7] text-sm px-6 py-[17px] xarta-footer">
            <div className="w-full flex flex-col card-minimum:flex-row">
                <div className="flex flex-col grow">
                    <p className="">
                        {post.meta_description ? <><span className="font-semibold text-black">Fonte:</span> {post.meta_description}</> : null}
                    </p>
                    <p className="mt-2"><span className="font-semibold">Criado por:</span> {/*post.primary_author.name*/} {settings.title} {" "} {post.featured ? <span>- <a className="text-[#4b31dd] underline" href={`${ROOT_URL}/xarta/view-card/${post.id}` ?? ''} target="_blank">Reprodução permitida
                    ...
                    <p className="text-left meta-date xarta-updated-at mt-2" suppressHydrationWarning><span className="font-semibold">Última atualização:</span> {formattedDate}</p>
